...
class NoteService:
    ...
    def resolve_context(
        ...
        *,
        ...
        """
        if note_id:
            note_docs = db.query(Document.id).filter(Document.note_id == note_id).all()
        ...
        if document_ids:
            ...
            for raw_id in document_ids:
                doc = db.query(Document).filter(Document.id == raw_id).first()
                if doc:
                    same_note_docs = (
                ...
                possible_note_docs = db.query(Document.id).filter(Document.note_id == raw_id).all()
                expanded.extend([row[0] for row in possible_note_docs])

            doc_ids = _dedupe(expanded)
            context_id = ""
            if doc_ids:
                first_doc = db.query(Document).filter(Document.id == doc_ids[0]).first()
                context_id = first_doc.note_id if first_doc and first_doc.note_id else doc_ids[0]
            return doc_ids, context_id

        if topic:
            topic_docs = db.query(Document).filter(Document.topic == topic).all()
        ...
        if document_id:
            ...
            if doc:
                note_docs = db.query(Document.id).filter(Document.note_id == doc.note_id).all()
                doc_ids = _dedupe([row[0] for row in note_docs])
                context_id = doc.note_id if doc.note_id else document_id
                return doc_ids, context_id

            possible_note_docs = db.query(Document.id).filter(Document.note_id == document_id).all()
    ...
    @staticmethod
    def resolve_context_id_from_any_id(db: Session, any_id: str) -> str:
        doc = db.query(Document).filter(Document.id == any_id).first()
